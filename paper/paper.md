...
The metadata annotation of the example datasets on worked well due to the semantical closeness of crop entities to Bioschemas' ['BioSample'](https://bioschemas.org/BioSample) type. To evaluate the feasibility of the ideas developed during the hackathon, additional example datasets will be annotated in the future. They will include entities such as soils, pesticides, diseases and sensors to ensure a diverse set of research domains. The planned metadata annotation will explore the limits of the developed approach. Depending on the results a combination of both presented approaches might make sense: Reuse available resources in a flexible way and close gaps for remaining needs by extension. 
