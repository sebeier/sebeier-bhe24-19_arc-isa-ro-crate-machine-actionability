...
Project 19 focused on improving the machine-actionability of ARC containers packaged as RO-Crates. For simplification and practical reasons, we chose an RNA-Seq analysis with DESeq2 as a use case for the BioHackathon. The goal was to invoke a workflow on Galaxy that relies on metadata (in particular experimental factors) from an ARC and allows users to run analysis directly from a preconfigured setup. We successfully wrapped DESeq2 in CWL building on multi-package containers of the [BioContainers](https://github.com/BioContainers/multi-package-containers) community.
